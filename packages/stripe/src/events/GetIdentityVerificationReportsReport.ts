                    ...
                    export const GetIdentityVerificationReportsReport: EventHandler<StripeIntegration> = ({
  eventKey,
  ...
  makeWebhookUrl,
                        ...
                        executor: async ({ event, step }: any) => {
                            const { expand,report, report,  } = event.data;
                            const { referenceId } = event.user;
                            const proxy = await getProxy({ referenceId })

                         
                            const response = await proxy['/v1/identity/verification_reports/{report}'].get({
                                query: {expand,report,},
                                params: {report,} })
