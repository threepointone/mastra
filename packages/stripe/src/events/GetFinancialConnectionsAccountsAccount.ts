                    ...
                    export const GetFinancialConnectionsAccountsAccount: EventHandler<StripeIntegration> = ({
  eventKey,
  ...
  makeWebhookUrl,
                        ...
                        executor: async ({ event, step }: any) => {
                            const { account,expand, } = event.data;
                            const { referenceId } = event.user;
                            const proxy = await getProxy({ referenceId })


                            const response = await proxy['/v1/financial_connections/accounts/{account}'].get({
