                    ...
                    export const GetCustomersCustomerBalanceTransactionsTransaction: EventHandler<StripeIntegration> = ({
  eventKey,
  ...
  makeWebhookUrl,
                        ...
                        executor: async ({ event, step }: any) => {
                            const { customer,expand,transaction, customer,transaction,  } = event.data;
                            const { referenceId } = event.user;
                            const proxy = await getProxy({ referenceId })


                            const response = await proxy['/v1/customers/{customer}/balance_transactions/{transaction}'].get({
