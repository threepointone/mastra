                    ...
                    export const GetTreasuryFinancialAccountsFinancialAccount: EventHandler<StripeIntegration> = ({
  eventKey,
  ...
  makeWebhookUrl,
                        ...
                        executor: async ({ event, step }: any) => {
                            const { expand,financial_account, } = event.data;
                            const { referenceId } = event.user;
                            const proxy = await getProxy({ referenceId })


                            const response = await proxy['/v1/treasury/financial_accounts/{financial_account}'].get({
