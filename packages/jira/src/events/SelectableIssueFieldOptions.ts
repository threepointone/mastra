                    ...
                    export const SelectableIssueFieldOptions: EventHandler<JiraIntegration> = ({
  eventKey,
  ...
  makeWebhookUrl,
                        ...
                        executor: async ({ event, step }: any) => {
                            const { startAt,maxResults,projectId,fieldKey, } = event.data;
                            const { referenceId } = event.user;
                            const proxy = await getProxy({ referenceId })


                            const response = await proxy['/rest/api/3/field/{fieldKey}/option/suggestions/edit'].get({
