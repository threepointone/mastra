                    ...
                    export const NotificationSchemeForProject: EventHandler<JiraIntegration> = ({
  eventKey,
  ...
  makeWebhookUrl,
                        ...
                        executor: async ({ event, step }: any) => {
                            const { projectKeyOrId,expand, } = event.data;
                            const { referenceId } = event.user;
                            const proxy = await getProxy({ referenceId })


                            const response = await proxy['/rest/api/3/project/{projectKeyOrId}/notificationscheme'].get({
