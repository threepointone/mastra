...
export const getFieldsPaginated: EventHandler<JiraIntegration> = ({
  eventKey,
  ...
  executor: async ({ event, step }: any) => {
    ...
    const response = await proxy['/rest/api/3/field/search'].get({
      query: { startAt, maxResults, type, id, query, orderBy, expand },
    });
