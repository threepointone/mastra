                    ...
                    export const CompanySearchNumber: EventHandler<BrexIntegration> = ({
  eventKey,
  ...
  makeWebhookUrl,
                        ...
                        executor: async ({ event, step }: any) => {
                            const { country,number,limit, country,number,  } = event.data;
                            const { referenceId } = event.user;
                            const proxy = await getProxy({ referenceId })

                         
                            const response = await proxy['/api/v1/company/search/number/{country}/{number}'].get({
                                query: {country,number,limit,},
