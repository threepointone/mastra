                    ...
                    export const CompanyDeepsearchName: EventHandler<BrexIntegration> = ({
  eventKey,
  ...
  makeWebhookUrl,
                        ...
                        executor: async ({ event, step }: any) => {
                            const { country,name, country,name,  } = event.data;
                            const { referenceId } = event.user;
                            const proxy = await getProxy({ referenceId })

                         
                            const response = await proxy['/api/v1/company/deepsearch/name/{country}/{name}'].get({
                                query: {country,name,},
                                params: {country,name,} })
