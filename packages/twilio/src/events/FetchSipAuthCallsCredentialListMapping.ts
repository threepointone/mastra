  ...
  makeWebhookUrl,
                        ...
                        executor: async ({ event, step }: any) => {
                            ...
                            if (!response.ok) {
                              console.log("error in fetching FetchSipAuthCallsCredentialListMapping", {response});
                              return
                            }
                            ...
                            const records = d?.data?.map(({ _externalId, ...d2 }) => ({
                                ...
                                data: d2,
                                entityType: `api_v2010_account_sip_sip_domain_sip_auth_sip_auth_calls_sip_auth_calls_credential_list_mapping`,
                            }));
