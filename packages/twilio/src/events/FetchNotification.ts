...
export const FetchNotification: EventHandler<TwilioIntegration> = ({
  ...
  executor: async ({ event, step }: any) => {
    ...
    if (!response.ok) {
      console.log('error in fetching FetchNotification', { response });
      return;
    }
    ...
    const records = d?.data?.map(({ _externalId, ...d2 }) => ({
      ...
      data: d2,
      entityType: `api_v2010_account_notification_instance`,
    }));
    ...
    await dataLayer?.syncData({
      ...
      referenceId,
      data: records,
      type: `api_v2010_account_notification_instance`,
      properties: api_v2010_account_notification_instanceFields,
    });
  },
});
