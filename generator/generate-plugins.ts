...
async function main() {
    ...
    const apiProvidersRes = await fetch('https://api.apis.guru/v2/providers.json')

    const apiProviderResult = await apiProvidersRes.json()
    ...
    for (const entry of entries) {
        const uri = new URL(entry.authorization_url)

        const hasIt = apiProviderResult.data.find((p) => {
            return new RegExp(uri.host).test(p) || new RegExp(entry.name).test(p)
        })

        if (hasIt) {
            const providerRes = await fetch(`https://api.apis.guru/v2/${hasIt}.json`)
            const providerData = await providerRes.json()
            console.log(providerData.apis)
            // Generate TS Client
        }
    }
